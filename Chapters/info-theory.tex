...
$$
H(X|Y) = \sum_{y \in \mathcal Y} \Prob_{\mathcal Y}(y) H(X| Y = y) = \sum{y \in \mathcal Y} \Prob_{\mathcal  Y}(y) \sum_{x \in \X} \Prob_{X | Y}(x|y)\log\frac{1}{\Prob_{X|Y}(x|y)} = \sum_{x \in X,y \in \mathcal Y}\Prob_{XY}(x,y)\log\frac{P_Y(y)}{P_{XY}(x,y)}
$$
\end{ndef}
