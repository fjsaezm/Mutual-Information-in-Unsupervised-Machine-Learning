...
\begin{ndef}
Let $\D$ be a dataset of independent observatoins and $\theta$ a set of parameters. Then, we define the log-likelyhood $\mathcal l$ as the sum of the logarithms of the evaluations of $p_\theta$ in each $x$ in the dataset. That is:
\[
\mathcal l(\theta | X) = \sum_{x \in \D} \log p_\theta(x).
\]
\end{ndef}
...
\begin{ndef}
We say that \emph{maximum likelyhood estimation} is the method used
\end{ndef}
