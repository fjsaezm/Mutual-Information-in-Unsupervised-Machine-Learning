...
\item If $X,Z$ are random variables, then its mutual information equals zero if and only if they are independent. This is trivial because if $D_{KL}(P_{XZ} \ || \ P_X P Z) = 0$, then $P_{XZ} = P_X P_Z$ almost everywhere and,so, $X$ and $Z$ are independent.
